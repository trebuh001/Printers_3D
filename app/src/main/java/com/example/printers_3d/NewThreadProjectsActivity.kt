...
import kotlinx.android.synthetic.main.activity_new_thread_projects.*
...
class NewThreadProjectsActivity : AppCompatActivity() {
    var generatePostID = ""
    val TAG="NewThreadProjects TAG"
    public var i=0
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_new_thread_projects)
                ...
                DialogInterface.BUTTON_POSITIVE -> {
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        if (checkSelfPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE) ==
                            ...
                            requestPermissions(permissions, NewThreadProjectsActivity.PERMISSION_CODE);
                        } else {
                            //permission already granted
                            val intent = Intent(Intent.ACTION_PICK)
                            intent.type = "image/*"
                            startActivityForResult(intent, NewThreadProjectsActivity.IMAGE_PICK_CODE)
                        }
                    } else {
                        val intent = Intent(Intent.ACTION_PICK)
                        intent.type = "image/*"
...
//            Toast.makeText(applicationContext,imageURL,Toast.LENGTH_LONG).show()
    ...
    override fun onRequestPermissionsResult(
        ...
        when(requestCode){
            NewThreadProjectsActivity.PERMISSION_CODE -> {
    ...
    fun SaveThreadButton(v: View) {
        ...
        save_thread_projects.isEnabled=false
        if (Et_new_thread_title.text.trim().isEmpty())
        {
            ...
            save_thread_projects.isEnabled=true
            return
        }
        ...
        projects_new_thread_scroll.scrollTo(0,0)
        ...
        //    {
        //       this@NewThreadProjectsActivity.name =ds.child("name").getValue(String::class.java).toString()
   ...
   //     imageBigPicture.clear()
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if(resultCode == Activity.RESULT_OK && requestCode == NewThreadProjectsActivity.IMAGE_PICK_CODE) {
